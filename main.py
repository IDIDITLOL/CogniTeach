...
def emotion_detection(url,online=True):
    ...
    if online:
            ...
            results = predictor.classify_image_url(project_id=UUID("6c0ac084-adb6-4a3b-945c-b0b8f4cb6c4d"),published_name="Iteration3",url=url)
            return results.predictions[0].tag_name
    else:
            results = predictor.classify_image(UUID("6c0ac084-adb6-4a3b-945c-b0b8f4cb6c4d"), "Iteration3", url)
            return results.predictions[0].tag_name
...
def capture():
    ...
    with open('captured_image.png', "rb") as image_contents:
        predictions = predictor.classify_image(UUID("6c0ac084-adb6-4a3b-945c-b0b8f4cb6c4d"), "Iteration3", image_contents.read())
